...
app.layout = html.Div(
        ...
        html.Div(
            [
                ...
                html.Div(
                    ...
                    style={"flex": "1", "minWidth": "360px", "padding": "0.5rem 1rem 1rem 0.5rem"},
                ),
            ],
            style={
                "display": "flex",
    ...
    style={"fontFamily": "Arial, sans-serif", "maxWidth": "1900px", "margin": "0 auto"},
)
...
def build_event_count_figure(df: pd.DataFrame, selected_year: int, selected_people=None) -> go.Figure:
    ...
    colors = ["#c0392b" if int(y) == int(selected_year) else "#9aa5b1" for y in counts["year"]]
...
def aggregate_year_locations(events_df: pd.DataFrame, selected_year: int) -> pd.DataFrame:
    if events_df.empty:
        return pd.DataFrame()

    current = events_df[events_df["year"] == selected_year].copy()
    if current.empty:
        return pd.DataFrame()
    ...
    for (lat, lon, location_label), group in current.groupby(
        ...
        hover_lines = [
            ...
            f"People currently here: {people['person_id'].nunique()}",
            f"Events here this year: {len(events)}",
            "",
            "<b>People overview</b>",
        ]

        for _, row in visits_so_far.iterrows():
...
def add_year_location_markers(fig, grouped_df: pd.DataFrame):
    ...
    fig.add_trace(
        go.Scattermap(
            ...
            customdata=grouped_df[["latitude", "longitude", "location_label", "year"]].values,
            name="Events in selected year",
            showlegend=True,
        )
    )
...
def build_selected_timeline(all_visible_df: pd.DataFrame, selected_people):
    ...
    if selected_df.empty:
        return html.Div(
            "No visible events for the selected people at this year.",
            style={"color": "#666"},
        )
...
def summarize_location_year(events_df: pd.DataFrame, selected_year: int, lat: float, lon: float):
    current = events_df[
        (events_df["year"] == selected_year)
        & (events_df["latitude"] == lat)
        & (events_df["longitude"] == lon)
...
def sync_year_from_bar(click_data, current_year):
...
def store_clicked_location(click_data, selected_year):
    if not click_data or not click_data.get("points"):
        return no_update
    ...
    lat, lon, location_label, year = customdata
    return {
        "lat": float(lat),
        "lon": float(lon),
        "location_label": location_label,
...
def update_hover_summary(hover_data, selected_year, selected_people):
    default_msg = html.Div(
        "Hover over a black location marker to see who is there and how often they have visited.",
        style={"color": "#666"},
    )
    ...
    summary = summarize_location_year(base_df, int(year), float(lat), float(lon))
    if not summary:
    ...
    return html.Div(
        [
            ...
            html.Div(f"Year: {summary['year']}"),
            html.Div(f"People currently there: {len(summary['people_now'])}"),
            html.Div(f"Events there this year: {len(summary['current_events'])}"),
            html.Div("People overview", style={"marginTop": "0.5rem", "fontWeight": "bold"}),
            html.Ul(lines, style={"marginTop": "0.35rem"}),
            html.Div(
                "Click this location marker to browse each person's events there so far.",
...
def update_location_person_events(clicked_location, person_id):
    ...
    selected_year = clicked_location["year"]
    ...
    return html.Div(
        [
            html.Div(f"{person_name} — events here so far", style={"fontWeight": "bold"}),
            html.Div(f"Visits up to {selected_year}: {visit_count}", style={"marginBottom": "0.5rem", "color": "#666"}),
            html.Div(rows),
        ]
    )
...
def update_map(selected_year, selected_people, display_mode, direction_mode, background_opacity):
    if DF.empty:
        empty_bar = build_event_count_figure(pd.DataFrame(columns=["year", "person_id"]), selected_year, selected_people)
        return (
            empty_bar,
    ...
    selected_people = selected_people or []
    visible_df = DF[DF["year"] <= selected_year].copy()
    year_df = DF[DF["year"] == selected_year].copy()

    bar_fig = build_event_count_figure(DF, selected_year, selected_people)

    if visible_df.empty:
        return (
            bar_fig,
            empty_figure("No events available up to this year."),
            f"No events available up to {selected_year}.",
            [],
            html.Div("No events available for this year.", style={"color": "#666"}),
        )
    ...
    if selected_people:
        ...
        add_year_location_markers(fig, aggregate_year_locations(highlighted, selected_year))
    else:
        add_event_markers(fig, visible_df, set())
        add_year_location_markers(fig, aggregate_year_locations(visible_df, selected_year))
    ...
    if selected_people:
        ...
        summary = html.Div(
            [
                html.Div(f"Year: {selected_year}"),
                html.Div(f"Selected people: {len(selected_people)}"),
                html.Div(f"Visible events for selection: {len(selection_df)}"),
                html.Div(
                    f"Events exactly in selected year: "
                    f"{len(year_df[year_df['person_id'].isin(selected_people)])}"
                ),
            ]
        )
        table_df = (
        ...
        summary = html.Div(
            [
                ...
                html.Div(f"Visible geocoded events up to year: {len(visible_df)}"),
                html.Div(f"Events exactly in selected year: {len(year_df)}"),
            ]
        )
        table_df = (
